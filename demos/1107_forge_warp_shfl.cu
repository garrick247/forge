...
typedef uint32_t (*forge_fn_u32_u32_u32_ret_u32_t)(uint32_t, uint32_t, uint32_t);
...
__device__ uint32_t shfl_xor_sync_u32(uint32_t val, uint32_t mask, uint32_t width);  /* extern: forge_gpu */
...
__global__ void vec_shfl_pair(forge_span_u32_t a __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t n __attribute__((unused)));
int main();
...
__global__ void vec_shfl_pair(forge_span_u32_t a __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t n __attribute__((unused))) {
  ...
  if ((tid < n)) {
    uint32_t x __attribute__((unused)) = a.data[tid];
    uint32_t y __attribute__((unused)) = shfl_xor_sync_u32(x, 1ULL, 32ULL);
    out.data[tid] = (x + y);
