...
__global__ void stencil_5pt(forge_span_u32_t inp __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t width __attribute__((unused)), uint32_t height __attribute__((unused)));
int main();
...
__global__ void stencil_5pt(forge_span_u32_t inp __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t width __attribute__((unused)), uint32_t height __attribute__((unused))) {
  ...
  uint32_t bx __attribute__((unused)) = blockIdx_x;
  uint32_t by __attribute__((unused)) = blockIdx_y;
  uint32_t col __attribute__((unused)) = ((bx * 16ULL) + x);
  uint32_t row __attribute__((unused)) = ((by * 16ULL) + y);
  if ((col > 0ULL)) {
    if ((row > 0ULL)) {
      if (((col + 1ULL) < width)) {
        if (((row + 1ULL) < height)) {
          uint32_t idx __attribute__((unused)) = ((row * width) + col);
          uint32_t idx_n __attribute__((unused)) = (idx - width);
          uint32_t idx_s __attribute__((unused)) = (idx + width);
          uint32_t idx_w __attribute__((unused)) = (idx - 1ULL);
          uint32_t idx_e __attribute__((unused)) = (idx + 1ULL);
          if ((idx_s < inp.len)) {
            uint32_t center __attribute__((unused)) = inp.data[idx];
            uint32_t north __attribute__((unused)) = inp.data[idx_n];
            uint32_t south __attribute__((unused)) = inp.data[idx_s];
            uint32_t west __attribute__((unused)) = inp.data[idx_w];
            uint32_t east __attribute__((unused)) = inp.data[idx_e];
