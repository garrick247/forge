...
uint32_t merkle_compress(uint32_t a __attribute__((unused)), uint32_t b __attribute__((unused)));
uint32_t combine_first(uint32_t node __attribute__((unused)), uint32_t sib __attribute__((unused)), uint64_t i __attribute__((unused)));
uint32_t combine_second(uint32_t node __attribute__((unused)), uint32_t sib __attribute__((unused)), uint64_t i __attribute__((unused)));
uint32_t merkle_verify_path(uint32_t leaf __attribute__((unused)), uint64_t leaf_idx __attribute__((unused)), forge_span_u32_t siblings __attribute__((unused)), uint64_t depth __attribute__((unused)));
int main();
...
uint32_t merkle_verify_path(uint32_t leaf __attribute__((unused)), uint64_t leaf_idx __attribute__((unused)), forge_span_u32_t siblings __attribute__((unused)), uint64_t depth __attribute__((unused))) {
  uint32_t node __attribute__((unused)) = leaf;
  uint64_t idx __attribute__((unused)) = leaf_idx;
  uint64_t l __attribute__((unused)) = 0ULL;
  {
    while ((l < depth)) {
      /* assert erased */;
      /* assert erased */;
      uint32_t sib __attribute__((unused)) = siblings.data[l];
      uint32_t a __attribute__((unused)) = combine_first(node, sib, idx);
      uint32_t b __attribute__((unused)) = combine_second(node, sib, idx);
      node = merkle_compress(a, b);
      idx = (idx / 2ULL);
      l = (l + 1ULL);
    }
