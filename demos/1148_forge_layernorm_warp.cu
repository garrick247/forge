...
/* ---- FORGE ASSUMPTION AUDIT LOG ----
   Total assumptions: 3
   [ASSUME] test/../demos/1148_forge_layernorm_warp.fg:34  (not (= n_f 0))
             "n_cols >= 1 (precondition) implies (n_cols as f32) != 0"
   [ASSUME] test/../demos/1148_forge_layernorm_warp.fg:38  (< (+ base lane) inp__len)
             "row<gridDim + lane<n_cols + gridDim*n_cols<=inp.len"
   [ASSUME] test/../demos/1148_forge_layernorm_warp.fg:59  (< (+ base lane) out__len)
             "mirrors inp bounds"
   ---- END AUDIT LOG ---- */
