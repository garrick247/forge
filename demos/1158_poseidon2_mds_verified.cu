...
static __device__ __forceinline__ uint32_t poseidon2_mds3_out0(uint32_t s0, uint32_t s1, uint32_t s2) {
  uint32_t d = m31_double(s0);
  uint32_t t = m31_add(s1, s2);
  0 /* unhandled */;
  return m31_add(d, t);
}

static __device__ __forceinline__ uint32_t poseidon2_mds3_out1(uint32_t s0, uint32_t s1, uint32_t s2) {
  uint32_t d = m31_double(s1);
  uint32_t t = m31_add(s0, s2);
  0 /* unhandled */;
  return m31_add(d, t);
}

static __device__ __forceinline__ uint32_t poseidon2_mds3_out2(uint32_t s0, uint32_t s1, uint32_t s2) {
  uint32_t d = m31_double(s2);
  uint32_t t = m31_add(s0, s1);
  0 /* unhandled */;
  return m31_add(d, t);
}

static __device__ __forceinline__ uint32_t poseidon2_add_rc(uint32_t s, uint32_t rc) {
  return m31_add(s, rc);
}
