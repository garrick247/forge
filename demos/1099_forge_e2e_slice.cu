...
static __host__ __device__ __forceinline__ uint32_t compute_clamp_u32(uint32_t a __attribute__((unused)), uint32_t b __attribute__((unused)));
__global__ void vec_compute_clamp(forge_span_u32_t a __attribute__((unused)), forge_span_u32_t b __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t n __attribute__((unused)));
int main();
...
static __host__ __device__ __forceinline__ uint32_t compute_clamp_u32(uint32_t a __attribute__((unused)), uint32_t b __attribute__((unused))) {
  uint32_t x __attribute__((unused)) = (a + b);
  uint32_t y __attribute__((unused)) = (x ^ 3735928559ULL);
  uint32_t z __attribute__((unused)) = (y * 3ULL);
  if ((z > 1024ULL)) {
    return 1024ULL;
  } else {
    return z;
...
__global__ void vec_compute_clamp(forge_span_u32_t a __attribute__((unused)), forge_span_u32_t b __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t n __attribute__((unused))) {
  ...
  if ((tid < n)) {
    out.data[tid] = compute_clamp_u32(a.data[tid], b.data[tid]);
