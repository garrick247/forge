...
typedef uint32_t (*forge_fn_ptr_u32_u32_ret_u32_t)(uint32_t*, uint32_t);
...
__device__ uint32_t atom_add_u32(uint32_t* ptr, uint32_t val);  /* extern: forge_gpu */
...
__global__ void vec_block_reduce(forge_span_u32_t a __attribute__((unused)), uint32_t* out_ptr __attribute__((unused)), uint32_t n __attribute__((unused)), uint32_t n_minus_1 __attribute__((unused)));
int main();
...
__global__ void vec_block_reduce(forge_span_u32_t a __attribute__((unused)), uint32_t* out_ptr __attribute__((unused)), uint32_t n __attribute__((unused)), uint32_t n_minus_1 __attribute__((unused))) {
  ...
  uint32_t gid __attribute__((unused)) = ((blockIdx_x * blockDim_x) + threadIdx_x);
  if ((gid < n)) {
    smem[tid] = a.data[gid];
  ...
  __syncthreads();
  if ((tid == 0ULL)) {
    uint32_t _old __attribute__((unused)) = atom_add_u32(out_ptr, smem[0ULL]);
