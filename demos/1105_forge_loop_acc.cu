...
__global__ void vec_loop_acc(forge_span_u32_t a __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t n __attribute__((unused)), uint32_t n_minus_2 __attribute__((unused)));
int main();
...
__global__ void vec_loop_acc(forge_span_u32_t a __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t n __attribute__((unused)), uint32_t n_minus_2 __attribute__((unused))) {
  ...
  if ((tid < n_minus_2)) {
    uint32_t sum __attribute__((unused)) = 0ULL;
    uint32_t i __attribute__((unused)) = 0ULL;
    {
      while ((i < 3ULL)) {
        sum = (sum + a.data[(tid + i)]);
        i = (i + 1ULL);
      }
