...
__global__ void transpose_tile(forge_span_u32_t inp __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t width __attribute__((unused)), uint32_t height __attribute__((unused)));
int main();
...
__global__ void transpose_tile(forge_span_u32_t inp __attribute__((unused)), forge_span_u32_t out __attribute__((unused)), uint32_t width __attribute__((unused)), uint32_t height __attribute__((unused))) {
  ...
  uint32_t gx __attribute__((unused)) = ((bx * 16ULL) + x);
  uint32_t gy __attribute__((unused)) = ((by * 16ULL) + y);
  if ((gx < width)) {
    if ((gy < height)) {
      uint32_t in_idx __attribute__((unused)) = ((gy * width) + gx);
      uint32_t sw_idx __attribute__((unused)) = ((y * 16ULL) + x);
      smem[sw_idx] = inp.data[in_idx];

    }

  }
  __syncthreads();
  uint32_t tx __attribute__((unused)) = ((by * 16ULL) + x);
  uint32_t ty __attribute__((unused)) = ((bx * 16ULL) + y);
  if ((tx < height)) {
    if ((ty < width)) {
      uint32_t out_idx __attribute__((unused)) = ((ty * height) + tx);
      uint32_t sr_idx __attribute__((unused)) = ((x * 16ULL) + y);
      if ((out_idx < out.len)) {
        out.data[out_idx] = smem[sr_idx];
