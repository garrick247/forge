...
__global__ void circle_ntt_warp_forward(forge_span_u32_t data __attribute__((unused)), forge_span_u32_t twiddles __attribute__((unused)), uint32_t layer_idx __attribute__((unused)));
__global__ void circle_ntt_warp_inverse(forge_span_u32_t data __attribute__((unused)), forge_span_u32_t twiddles __attribute__((unused)), uint32_t layer_idx __attribute__((unused)));
int main();
...
__global__ void circle_ntt_warp_forward(forge_span_u32_t data __attribute__((unused)), forge_span_u32_t twiddles __attribute__((unused)), uint32_t layer_idx __attribute__((unused))) {
  ...
  uint64_t h __attribute__((unused)) = (tid >> ((uint64_t)(layer_idx + 1ULL)));
  if ((tid < data.len)) {
    if ((h < twiddles.len)) {
      uint32_t val __attribute__((unused)) = reduce_word(data.data[tid]);
      uint32_t t __attribute__((unused)) = reduce_word(twiddles.data[h]);
      uint64_t partner_u64 __attribute__((unused)) = __shfl_xor_sync(0xffffffff, ((uint64_t)val), stride, 32ULL);
      uint32_t partner __attribute__((unused)) = reduce_word(((uint32_t)partner_u64));
      uint32_t new_val;
      if (((tid & stride) == 0ULL)) {
        uint32_t prod __attribute__((unused)) = m31_mul(partner, t);
        new_val = m31_add(val, prod);
      } else {
        uint32_t prod __attribute__((unused)) = m31_mul(val, t);
        new_val = m31_sub(partner, prod);
      }
      data.data[tid] = new_val;
...
__global__ void circle_ntt_warp_inverse(forge_span_u32_t data __attribute__((unused)), forge_span_u32_t twiddles __attribute__((unused)), uint32_t layer_idx __attribute__((unused))) {
  ...
  if ((tid < data.len)) {
    if ((h < twiddles.len)) {
      ...
      uint32_t new_val;
      if (((tid & stride) == 0ULL)) {
        new_val = m31_add(val, partner);
      } else {
        uint32_t diff __attribute__((unused)) = m31_sub(partner, val);
        new_val = m31_mul(diff, t);
      }
      data.data[tid] = new_val;
