...
static __device__ __forceinline__ uint32_t reduce_word(uint32_t v) {
  uint32_t lo = (v & M31_P);
  uint32_t hi = (v >> 31U);
  uint32_t r = (lo + hi);
  if ((r >= M31_P)) {
...
__global__ void eval_fold_first(uint32_t* __restrict__ coeffs, uint64_t coeffs_len, uint32_t* __restrict__ out, uint64_t out_len, uint32_t fa, uint32_t fb, uint32_t fc, uint32_t fd, uint64_t half_n) {
  uint64_t i = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((i < half_n)) {
    uint64_t bi = (i + half_n);
    uint64_t o3 = ((i * 4ULL) + 3ULL);
    if ((bi < coeffs_len)) {
      if ((o3 < out_len)) {
        uint32_t a = reduce_word(coeffs[i]);
        uint32_t b = reduce_word(coeffs[bi]);
        uint32_t bf0 = m31_mul(fa, b);
        uint32_t bf1 = m31_mul(fb, b);
        uint32_t bf2 = m31_mul(fc, b);
        uint32_t bf3 = m31_mul(fd, b);
        out[((i * 4ULL) + 0ULL)] = m31_add(a, bf0);
        out[((i * 4ULL) + 1ULL)] = bf1;
        out[((i * 4ULL) + 2ULL)] = bf2;
        out[((i * 4ULL) + 3ULL)] = bf3;
      }
...
__global__ void eval_fold_level(uint32_t* __restrict__ in_data, uint64_t in_data_len, uint32_t* __restrict__ out_data, uint64_t out_data_len, uint32_t fa, uint32_t fb, uint32_t fc, uint32_t fd, uint64_t half_n) {
  uint64_t i = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((i < half_n)) {
    uint64_t ai = (i * 4ULL);
    uint64_t bi = ((i + half_n) * 4ULL);
    uint64_t a3 = (ai + 3ULL);
    uint64_t b3 = (bi + 3ULL);
    if ((a3 < in_data_len)) {
      if ((b3 < in_data_len)) {
        if ((a3 < out_data_len)) {
          uint32_t a0 = reduce_word(in_data[(ai + 0ULL)]);
          uint32_t a1 = reduce_word(in_data[(ai + 1ULL)]);
          uint32_t a2 = reduce_word(in_data[(ai + 2ULL)]);
          uint32_t a3v = reduce_word(in_data[(ai + 3ULL)]);
          uint32_t b0 = reduce_word(in_data[(bi + 0ULL)]);
          uint32_t b1 = reduce_word(in_data[(bi + 1ULL)]);
          uint32_t b2 = reduce_word(in_data[(bi + 2ULL)]);
          uint32_t b3v = reduce_word(in_data[(bi + 3ULL)]);
          uint32_t p0 = qm31_mul_out_re_re(fa, fb, fc, fd, b0, b1, b2, b3v);
          uint32_t p1 = qm31_mul_out_re_im(fa, fb, fc, fd, b0, b1, b2, b3v);
          uint32_t p2 = qm31_mul_out_im_re(fa, fb, fc, fd, b0, b1, b2, b3v);
          uint32_t p3 = qm31_mul_out_im_im(fa, fb, fc, fd, b0, b1, b2, b3v);
          out_data[(ai + 0ULL)] = qm31_add_re_re(a0, p0);
          out_data[(ai + 1ULL)] = qm31_add_re_im(a1, p1);
          out_data[(ai + 2ULL)] = qm31_add_im_re(a2, p2);
          out_data[(ai + 3ULL)] = qm31_add_im_im(a3v, p3);
        }
      }
    }
  }
