...
__global__ void fold_circle_into_line_soa(uint32_t* __restrict__ dst0, uint64_t dst0_len, uint32_t* __restrict__ dst1, uint64_t dst1_len, uint32_t* __restrict__ dst2, uint64_t dst2_len, uint32_t* __restrict__ dst3, uint64_t dst3_len, uint32_t* __restrict__ src0, uint64_t src0_len, uint32_t* __restrict__ src1, uint64_t src1_len, uint32_t* __restrict__ src2, uint64_t src2_len, uint32_t* __restrict__ src3, uint64_t src3_len, uint32_t* __restrict__ twiddles, uint64_t twiddles_len, uint32_t alpha_a, uint32_t alpha_b, uint32_t alpha_c, uint32_t alpha_d, uint32_t alpha_sq_a, uint32_t alpha_sq_b, uint32_t alpha_sq_c, uint32_t alpha_sq_d, uint64_t half_n) {
            ...
            if ((i < twiddles_len)) {
              if ((i < dst0_len)) {
                if ((i < dst1_len)) {
                  if ((i < dst2_len)) {
                    if ((i < dst3_len)) {
                      uint32_t f0_a_re = reduce_word(src0[idx0]);
                      uint32_t f0_a_im = reduce_word(src1[idx0]);
                      uint32_t f0_b_re = reduce_word(src2[idx0]);
                      uint32_t f0_b_im = reduce_word(src3[idx0]);
                      uint32_t f1_a_re = reduce_word(src0[idx1]);
                      uint32_t f1_a_im = reduce_word(src1[idx1]);
                      uint32_t f1_b_re = reduce_word(src2[idx1]);
                      uint32_t f1_b_im = reduce_word(src3[idx1]);
                      uint32_t tw = reduce_word(twiddles[i]);
                      uint32_t p_a_re = reduce_word(dst0[i]);
                      uint32_t p_a_im = reduce_word(dst1[i]);
                      uint32_t p_b_re = reduce_word(dst2[i]);
                      uint32_t p_b_im = reduce_word(dst3[i]);
                      ...
                      uint32_t fp_a_re = qm31_add_re_re(sum_a_re, at_a_re);
                      uint32_t fp_a_im = qm31_add_re_im(sum_a_im, at_a_im);
                      uint32_t fp_b_re = qm31_add_im_re(sum_b_re, at_b_re);
                      uint32_t fp_b_im = qm31_add_im_im(sum_b_im, at_b_im);
                      uint32_t pa_a_re = qm31_mul_out_re_re(p_a_re, p_a_im, p_b_re, p_b_im, alpha_sq_a, alpha_sq_b, alpha_sq_c, alpha_sq_d);
                      uint32_t pa_a_im = qm31_mul_out_re_im(p_a_re, p_a_im, p_b_re, p_b_im, alpha_sq_a, alpha_sq_b, alpha_sq_c, alpha_sq_d);
                      uint32_t pa_b_re = qm31_mul_out_im_re(p_a_re, p_a_im, p_b_re, p_b_im, alpha_sq_a, alpha_sq_b, alpha_sq_c, alpha_sq_d);
                      uint32_t pa_b_im = qm31_mul_out_im_im(p_a_re, p_a_im, p_b_re, p_b_im, alpha_sq_a, alpha_sq_b, alpha_sq_c, alpha_sq_d);
