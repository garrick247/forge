...
__global__ void circle_ntt_layer_forward(uint32_t* __restrict__ data, uint64_t data_len, uint32_t* __restrict__ twiddles, uint64_t twiddles_len, uint32_t layer_idx, uint64_t half_n) {
  uint64_t tid = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((tid < half_n)) {
    uint64_t stride = (1ULL << ((uint64_t)layer_idx));
    uint64_t h = (tid >> ((uint64_t)layer_idx));
    ...
    if ((idx0 < data_len)) {
      if ((idx1 < data_len)) {
        if ((h < twiddles_len)) {
          uint32_t v0 = reduce_word(data[idx0]);
          uint32_t v1 = reduce_word(data[idx1]);
...
__global__ void circle_ntt_layer_inverse(uint32_t* __restrict__ data, uint64_t data_len, uint32_t* __restrict__ twiddles, uint64_t twiddles_len, uint32_t layer_idx, uint64_t half_n) {
  uint64_t tid = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((tid < half_n)) {
    uint64_t stride = (1ULL << ((uint64_t)layer_idx));
    uint64_t h = (tid >> ((uint64_t)layer_idx));
