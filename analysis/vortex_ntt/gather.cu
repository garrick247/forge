...
__global__ void gather_u32(uint32_t* __restrict__ src, uint64_t src_len, uint32_t* __restrict__ idx, uint64_t idx_len, uint32_t* __restrict__ dst, uint64_t dst_len, uint64_t n, uint64_t src_len) {
  uint64_t i = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((i < n)) {
    uint32_t j = idx[i];
    if ((((uint64_t)j) < src_len)) {
      dst[i] = src[((uint64_t)j)];
    }
  }
}

__global__ void gather_u256(uint32_t* __restrict__ src, uint64_t src_len, uint32_t* __restrict__ idx, uint64_t idx_len, uint32_t* __restrict__ dst, uint64_t dst_len, uint64_t n, uint64_t src_len) {
  uint64_t i = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((i < n)) {
    uint32_t j = idx[i];
    uint64_t src_off = (((uint64_t)j) * 8ULL);
    uint64_t dst_off = (i * 8ULL);
    if (((src_off + 7ULL) < src_len)) {
      if (((dst_off + 7ULL) < dst_len)) {
        dst[dst_off] = src[src_off];
        dst[(dst_off + 1ULL)] = src[(src_off + 1ULL)];
        dst[(dst_off + 2ULL)] = src[(src_off + 2ULL)];
        dst[(dst_off + 3ULL)] = src[(src_off + 3ULL)];
        dst[(dst_off + 4ULL)] = src[(src_off + 4ULL)];
        dst[(dst_off + 5ULL)] = src[(src_off + 5ULL)];
        dst[(dst_off + 6ULL)] = src[(src_off + 6ULL)];
        dst[(dst_off + 7ULL)] = src[(src_off + 7ULL)];
      }
    }
  }
}
