...
__device__ uint32_t m31_inv(uint32_t a) {
  uint32_t acc = 1U;
  uint32_t base = a;
  uint32_t exp = (M31_P - 2U);
  while ((exp > 0U)) {
    uint32_t acc_next = m31_mul(acc, base);
    uint32_t base_next = m31_mul(base, base);
    if (((exp & 1U) == 1U)) {
      acc = acc_next;
    }
    base = base_next;
...
__global__ void batch_inverse_m31(uint32_t* __restrict__ input, uint64_t input_len, uint32_t* __restrict__ output, uint64_t output_len, uint64_t n) {
  uint64_t tid = ((blockIdx.x * blockDim.x) + threadIdx.x);
  uint64_t chunk = 64ULL;
  uint64_t start = (tid * chunk);
  if ((start < n)) {
    uint64_t raw_end = (start + chunk);
    uint64_t end = __if_stmt__;
    uint64_t count = (end - start);
    uint32_t prefix[64ULL] = 0 /* unhandled */;
    prefix[0ULL] = input[start];
    uint64_t i = 1ULL;
    while ((i < count)) {
      uint32_t val = input[(start + i)];
      uint32_t prev = prefix[(i - 1ULL)];
      uint32_t prod = m31_mul(prev, val);
      if ((val == 0U)) {
        prefix[i] = prev;
      } else {
        prefix[i] = prod;
    ...
    uint32_t total = prefix[(count - 1ULL)];
    uint32_t inv = m31_inv(total);
    uint64_t k = count;
    while ((k > 1ULL)) {
      uint64_t i2 = (k - 1ULL);
      uint32_t val = input[(start + i2)];
      uint32_t pre = prefix[(i2 - 1ULL)];
      if ((pre < M31_P)) {
        uint32_t out_val = m31_mul(inv, pre);
        uint32_t inv_next = m31_mul(inv, val);
        if ((val == 0U)) {
          output[(start + i2)] = 0U;
        } else {
    ...
    uint32_t v0 = input[start];
    output[start] = __if_stmt__;
  }
}
