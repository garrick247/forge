...
__global__ void permute_hc_to_canonic_brt(uint32_t* __restrict__ src, uint64_t src_len, uint32_t* __restrict__ dst, uint64_t dst_len, uint64_t n, uint32_t log_n) {
  uint64_t k = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((k < n)) {
    uint32_t cn = __if_stmt__;
    uint32_t j = bit_reverse_u32(cn, log_n);
    uint64_t j64 = ((uint64_t)j);
    if ((j64 < dst_len)) {
      dst[j64] = src[k];
    }
  }
...
__global__ void permute_canonic_brt_to_hc_natural(uint32_t* __restrict__ src, uint64_t src_len, uint32_t* __restrict__ dst, uint64_t dst_len, uint64_t n, uint32_t log_n) {
  uint64_t k = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((k < n)) {
    uint32_t cn = __if_stmt__;
    uint32_t j = bit_reverse_u32(cn, log_n);
