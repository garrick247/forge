...
__global__ void circle_ntt_warp_fused2_forward(forge_span_u32_t data __attribute__((unused)), forge_span_u32_t twiddles_a __attribute__((unused)), forge_span_u32_t twiddles_b __attribute__((unused)), uint32_t layer_idx __attribute__((unused)));
__global__ void circle_ntt_warp_fused2_inverse(forge_span_u32_t data __attribute__((unused)), forge_span_u32_t twiddles_a __attribute__((unused)), forge_span_u32_t twiddles_b __attribute__((unused)), uint32_t layer_idx __attribute__((unused)));
int main();
  ...
  if ((tid < data.len)) {
    if ((h_a < twiddles_a.len)) {
      if ((h_b < twiddles_b.len)) {
        ...
        uint32_t v1_raw;
        if (((tid & stride_a) == 0ULL)) {
          v1_raw = m31_add(v0, pa);
        } else {
          uint32_t diff __attribute__((unused)) = m31_sub(pa, v0);
          v1_raw = m31_mul(diff, t_a);
        }
        uint32_t v1 __attribute__((unused)) = reduce_word(v1_raw);
        uint64_t pb_u64 __attribute__((unused)) = __shfl_xor_sync(0xffffffff, ((uint64_t)v1), stride_b, 32ULL);
        uint32_t pb __attribute__((unused)) = reduce_word(((uint32_t)pb_u64));
        ...
        if (((tid & stride_b) == 0ULL)) {
          ...
          uint32_t diff __attribute__((unused)) = m31_sub(pb, v1);
          v2 = m31_mul(diff, t_b);
        }
        data.data[tid] = v2;
