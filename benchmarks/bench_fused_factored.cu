...
static int verify_correctness() {
    ...
    baby_bear_fused_perm_factored_kernel<<<1, 32>>>(forge_span_u32_t{out_d, 16}, forge_span_u32_t{state_d, 16}, 1);
...
static void bench(uint64_t n, int runs, int warmup) {
    ...
    for (int i = 0; i < warmup; i++) {
        baby_bear_fused_perm_factored_kernel<<<blocks, threads>>>(forge_span_u32_t{out_d, 16 * n}, forge_span_u32_t{state_d, 16 * n}, n);
    }
    CK(cudaDeviceSynchronize());
